...
Spectrum nee(Vector3 p, Vector3 omega, int current_medium, int bounces, Light light, Scene scene, pcg32_state& rng) {
    ...
    if (T_light > 0) {
        ...
        Spectrum Le = make_zero_spectrum();
        Le = emission(light, omega_prime, Real(0), p_prime, scene);
        Real pdf_nee = light_pmf(scene, light.index()) *
    ...
    while (true) {
        ...
        if (scatter) {
            ...
            if (next_dir_) {
                ...
                //Spectrum nee_out = nee(p, ray.dir, current_medium, bounces, scene.lights[sample_light(scene, next_pcg32_real<Real>(rng))], scene, rng);
                Spectrum nee_out = nee(p, -ray.dir, current_medium, bounces, scene.lights[sample_light(scene, next_pcg32_real<Real>(rng))], scene, rng);
