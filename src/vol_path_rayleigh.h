...
Spectrum vol_path_rayleigh_3(const Scene &scene,
...
Spectrum vol_path_rayleigh_4(const Scene &scene,
                            int x, int y, /* pixel coordinates */
                            pcg32_state &rng) {
    // Homework 2: implememt this!
    int w = scene.camera.width, h = scene.camera.height;
...
// with MIS between next event estimation and phase function sampling
// with surface lighting
Spectrum vol_path_rayleigh_6(const Scene &scene,
                          int x, int y, /* pixel coordinates */
                          pcg32_state &rng) {
    // Homework 2: implememt this!
    int w = scene.camera.width, h = scene.camera.height;
...
inline Spectrum calc_rayleigh_sigma_s(const Vector3& ray_org) {
    ...
    Real height = (distance(ray_org, Vector3{ 0, 0, 0 }));
    
    // convert to meter
    height = height * 6478 * 1000;
    //printf("height is: %lf \n", height);
...
// with MIS between next event estimation and phase function sampling
// with surface lighting
Spectrum vol_path_rayleigh(const Scene& scene,
    int x, int y, /* pixel coordinates */
    pcg32_state& rng) {
    // Homework 2: implememt this!
    int w = scene.camera.width, h = scene.camera.height;
    ...
    for (int bounces = 0; max_depth == -1 || bounces < max_depth; bounces++) {
        ...
        if (current_medium_id >= 0) {
            ...
            Spectrum sigma_t = calc_rayleigh_sigma_s(ray.org);
        ...
        Real w1 = 0;
        {
            ...
            if (max(T_light) > 0) {
                ...
                if (scatter) {
                    const Medium& medium = scene.media[current_medium_id];
                    //sigma_s = get_sigma_s(medium, vertex.position);
                    sigma_s = calc_rayleigh_sigma_s(vertex.position);
                }
                Spectrum L = emission(light, -dir_light, Real(0), point_on_light, scene);
                C1 = current_path_throughput * sigma_s * T_light * G * f * L / p1;
                // Multiple importance sampling: it's also possible
        ...
        if (scatter) {
            ...
            // Need to remember multiplying the scattering coefficient!
            //Spectrum sigma_s = get_sigma_s(medium, vertex.position);
            Spectrum sigma_s = calc_rayleigh_sigma_s(vertex.position);
            current_path_throughput *= sigma_s * (f / p2);
        }
        else {
            // BSDF sampling
